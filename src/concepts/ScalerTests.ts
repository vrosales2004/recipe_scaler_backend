...
const mockIngredientsForScaling = [
  ...
  { name: "Sugar", quantity: 100, unit: "g", scalingContext: "standard dry" },
  {
    ...
    quantity: 2,
    unit: "large",
    scalingContext: "discrete item, rounds up",
  }, // Expect rounding up
  {
    name: "Salt",
    quantity: 0.5,
    unit: "tsp",
    scalingContext: "to taste, delicate, scale less than linear",
  }, // Expect less than linear scaling
  {
    name: "Garlic Cloves",
    quantity: 3,
    unit: "cloves",
    scalingContext: "flavor, discrete, scales roughly linear but rounded",
  }, // Expect rounding
  {
    name: "Water",
    quantity: 250,
    unit: "ml",
    scalingContext: "standard liquid",
  },
  {
    name: "Chili Powder",
    quantity: 2,
    unit: "tbsp",
    scalingContext: "strong spice, scale less than linear",
  }, // Expect less than linear
];
...
function createMockLLMClient(): ILLMClient {
  return {
    executeLLM: async (prompt: string): Promise<string> => {
      ...
      const recipeMatch = prompt.match(
      ...
      if (originalRecipeData) {
        const { name, originalServings, targetServings, ingredients } =
          originalRecipeData;
        ...
        const scaled = ingredients.map((ing: any) => {
          ...
          if (
            ing.scalingContext?.toLowerCase().includes("taste") ||
            ing.scalingContext?.toLowerCase().includes("less than linear")
          ) {
            scaledQuantity = ing.quantity * Math.sqrt(scaleFactor); // e.g., square root scaling
            if (scaledQuantity < ing.quantity && scaledQuantity < 0.25) {
              scaledQuantity = 0.25; // Don't go too low
            }
            if (scaledQuantity > ing.quantity * 2) {
              scaledQuantity = ing.quantity * 2; // Cap increase
            }
          } else if (
            ing.scalingContext?.toLowerCase().includes(
              "discrete item, rounds up",
            )
          ) {
...
async function getLLMClient(): Promise<ILLMClient> {
  ...
  try {
    ...
    if (geminiApiKey) {
      console.log(
        `[LLM Client Setup] 'GEMINI_API_KEY' loaded from '${configPath}'.`,
      );
    }
  } catch (readError) {
    console.warn(
      `[LLM Client Setup] Could not read or parse 'config.json' (${
  ...
  if (!geminiApiKey) {
    console.warn(
      "WARNING: 'GEMINI_API_KEY' is not found in 'config.json' or environment variables. Using a MockLLMClient for AI scaling tests.",
    );
...
//     const recipeScalerConcept = new RecipeScalerConcept(
//       db,
//       recipeConcept,
//       llmClient,
//     );
...
//       });
//       assertNotEquals(
//         "error" in addRecipeResult,
//         true,
//         `Failed to add base recipe: ${
//           (addRecipeResult as { error: string }).error
//         }`,
//       );
...
//       const scaleAIResult = await recipeScalerConcept.scaleRecipeAI({
//         baseRecipeId,
//         targetServings,
//       });
//       assertNotEquals(
//         "error" in scaleAIResult,
//         true,
//         `AI scaling failed: ${(scaleAIResult as { error: string }).error}`,
//       );
//       const { scaledRecipeId } = scaleAIResult as {
//         scaledRecipeId: ScaledRecipe;
//       };
...
//       });
//       assertNotEquals(
//         "error" in fetchedScaledRecipe,
//         true,
//         `Failed to fetch scaled recipe: ${
//           (fetchedScaledRecipe as { error: string }).error
//         }`,
//       );
//       assertExists(
//         fetchedScaledRecipe,
//         "Scaled recipe should be retrievable by its ID.",
//       );
//       assertEquals(
//         (fetchedScaledRecipe as { _id: ScaledRecipe })._id,
//         scaledRecipeId,
//       );
//       assertEquals(
//         (fetchedScaledRecipe as { baseRecipeId: Recipe }).baseRecipeId,
//         baseRecipeId,
//       );
//       assertEquals(
//         (fetchedScaledRecipe as { targetServings: number }).targetServings,
//         targetServings,
//       );
//       assertEquals(
...
//       );
//       assert(
//         (fetchedScaledRecipe as { scaledIngredients: unknown[] })
//           .scaledIngredients.length > 0,
//         "Scaled ingredients list should not be empty.",
//       );

//       // Further checks on intelligent scaling based on mock LLM logic
//       const scaledIngredients = (fetchedScaledRecipe as {
...
//       const originalEggs = mockIngredientsForScaling.find((i) =>
//         i.name === "Eggs"
//       )!;
...
//         scaledEggs.quantity,
//         expectedEggs,
//         `AI scaled Eggs quantity (${scaledEggs.quantity}) should be ${expectedEggs} (rounded up).`,
//       );

//       // "Salt" (to taste, delicate, scale less than linear) - example of square root scaling
//       const originalSalt = mockIngredientsForScaling.find((i) =>
//         i.name === "Salt"
//       )!;
...
//       const sqrtScaledSalt = originalSalt.quantity *
//         Math.sqrt(linearScaleFactor); // 0.5 * sqrt(2) = 0.707
//       assert(
//         scaledSalt.quantity < linearScaledSalt,
//         `AI scaled Salt quantity (${scaledSalt.quantity}) should be strictly less than linear scaled (${linearScaledSalt}).`,
//       );
//       assertEquals(
//         scaledSalt.quantity,
//         parseFloat(sqrtScaledSalt.toFixed(2)),
//         "AI scaled Salt quantity should be sqrt scaled.",
//       ); // From mock LLM

//       // "Chili Powder" (strong spice, scale less than linear)
//       const originalChili = mockIngredientsForScaling.find((i) =>
//         i.name === "Chili Powder"
//       )!;
//       const scaledChili = scaledIngredients.find((i) =>
//         i.name === "Chili Powder"
//       )!;
//       const linearScaledChili = originalChili.quantity * linearScaleFactor; // 2 * 2 = 4
//       const sqrtScaledChili = originalChili.quantity *
//         Math.sqrt(linearScaleFactor); // 2 * sqrt(2) = 2.828
//       assert(
//         scaledChili.quantity < linearScaledChili,
//         `AI scaled Chili Powder quantity (${scaledChili.quantity}) should be strictly less than linear scaled (${linearScaledChili}).`,
//       );
//       assertEquals(
//         scaledChili.quantity,
//         parseFloat(sqrtScaledChili.toFixed(2)),
//         "AI scaled Chili Powder quantity should be sqrt scaled.",
//       ); // From mock LLM
...
//       assertEquals(
//         scaledFlour.quantity,
//         linearScaledFlour,
//         "AI scaled Flour should be linearly scaled.",
//       );
//     } finally {
//       await client.close();
//     }
...
Deno.test({
  ...
  async fn() {
    ...
    try {
      ...
      const manualScaleResult = await recipeScalerConcept.scaleManually({
        baseRecipeId,
        targetServings,
      });
      assertNotEquals(
        "error" in manualScaleResult,
        true,
        `Manual scaling failed: ${
          (manualScaleResult as { error: string }).error
        }`,
      );
      const { scaledRecipeId } = manualScaleResult as {
        scaledRecipeId: ScaledRecipe;
      };
      ...
      assertEquals(
        scaledFlour.quantity,
        originalFlour.quantity * scaleFactor,
        "Flour should be scaled linearly.",
      );

      const targetServingsDown = 1; // Scale down
      const manualScaleDownResult = await recipeScalerConcept.scaleManually({
        baseRecipeId,
        targetServings: targetServingsDown,
      });
      assertNotEquals("error" in manualScaleDownResult, true);
      const { scaledRecipeId: scaledRecipeIdDown } = manualScaleDownResult as {
        scaledRecipeId: ScaledRecipe;
      };
      assertExists(scaledRecipeIdDown);

      const fetchedScaledRecipeDown = await recipeScalerConcept
      ...
      assertEquals(
        scaledWater.quantity,
        parseFloat((originalWater.quantity * scaleFactorDown).toFixed(2)),
        "Water should be scaled linearly down.",
      );
    } finally {
      await client.close();
    }
...
Deno.test({
  ...
  async fn() {
    ...
    try {
      ...
      const result = await recipeScalerConcept.scaleManually({
        baseRecipeId: nonExistentRecipeId,
        targetServings: 4,
      });
      assertEquals("error" in result, true);
      assertEquals(
        (result as { error: string }).error,
        `Base recipe with ID ${nonExistentRecipeId} not found.`,
      );
    } finally {
      await client.close();
    }
...
Deno.test({
  ...
  async fn() {
    ...
    try {
      ...
      const resultZero = await recipeScalerConcept.scaleManually({
        baseRecipeId,
        targetServings: 0,
      });
      ...
      assertEquals(
        ...
        "targetServings must be greater than 0.",
      );

      const resultNegative = await recipeScalerConcept.scaleManually({
        baseRecipeId,
        targetServings: -2,
      });
...
Deno.test({
  ...
  async fn() {
    ...
    try {
      ...
      assertEquals(
        (result as { error: string }).error,
        `targetServings (${originalServings}) cannot be equal to originalServings (${originalServings}).`,
      );
    } finally {
      await client.close();
    }
...
// Deno.test({
//   name:
//     "scaleRecipeAI: should successfully scale a recipe using AI and store it",
...
//       assertEquals(
//         scaledSalt.quantity,
//         parseFloat(expectedSalt.toFixed(2)),
//         "AI scaled Salt quantity should be sqrt scaled.",
//       );
...
//       const expectedChili = originalChili.quantity *
//         Math.sqrt(linearScaleFactor); // 2 * sqrt(2.5) = 3.16
//       assert(
//         scaledChili.quantity < linearScaledChili,
//         `AI scaled Chili Powder quantity (${scaledChili.quantity}) should be strictly less than linear scaled (${linearScaledChili}).`,
//       );
//       assertEquals(
//         scaledChili.quantity,
//         parseFloat(expectedChili.toFixed(2)),
//         "AI scaled Chili Powder quantity should be sqrt scaled.",
//       );
...
Deno.test({
  ...
  async fn() {
    ...
    try {
      const nonExistentRecipeId = "recipe:fake-id" as Recipe;
      const result = await recipeScalerConcept.scaleRecipeAI({
        baseRecipeId: nonExistentRecipeId,
        targetServings: 4,
      });
      assertEquals("error" in result, true);
...
Deno.test({
  ...
  async fn() {
    ...
    try {
      ...
      const resultZero = await recipeScalerConcept.scaleRecipeAI({
        baseRecipeId,
        targetServings: 0,
      });
      assertEquals("error" in resultZero, true);
      ...
      const resultNegative = await recipeScalerConcept.scaleRecipeAI({
        baseRecipeId,
        targetServings: -2,
      });
      assertEquals("error" in resultNegative, true);
...
Deno.test({
  ...
  async fn() {
    ...
    try {
      ...
      const scaleResult = await recipeScalerConcept.scaleManually({
        baseRecipeId,
        targetServings: 4,
      });
      const { scaledRecipeId } = scaleResult as {
        scaledRecipeId: ScaledRecipe;
      };
      assertExists(scaledRecipeId);
...
Deno.test({
  ...
  async fn() {
    ...
    try {
      ...
      await recipeScalerConcept.scaleManually({
        baseRecipeId,
        targetServings: 6,
      });
      await recipeScalerConcept.scaleRecipeAI({
        baseRecipeId,
        targetServings: 9,
      });

      const fetched = await recipeScalerConcept._findScaledRecipe({
        baseRecipeId,
        targetServings: 6,
      });
...
Deno.test({
  ...
  async fn() {
    ...
    try {
      ...
      const scaledVersions = await recipeScalerConcept
        ._getScaledRecipesByBaseRecipe({ baseRecipeId });
      assertEquals(scaledVersions.length, 3);
      assert(
        scaledVersions.some((s) =>
          s.targetServings === 2 && s.scalingMethod === "manual"
        ),
      );
      assert(
        scaledVersions.some((s) =>
          s.targetServings === 8 && s.scalingMethod === "manual"
        ),
      );
      assert(
        scaledVersions.some((s) =>
          s.targetServings === 6 && s.scalingMethod === "manual"
        ),
      );
    } finally {
      await client.close();
